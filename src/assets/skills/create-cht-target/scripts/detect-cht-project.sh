...
if [[ -f "$PROJECT_PATH/targets.js" ]] || [[ -f "$PROJECT_PATH/tasks.js" ]] || [[ -f "$PROJECT_PATH/app_settings.json" ]] || [[ -d "$PROJECT_PATH/forms/app" ]]; then
    IS_CHT_PROJECT=true
fi
...
if [[ "$IS_CHT_PROJECT" == "false" ]]; then
    ...
    [[ ! -f "$PROJECT_PATH/targets.js" ]] && echo "  - targets.js"
...
# List existing targets.js content
if [[ -f "$PROJECT_PATH/targets.js" ]]; then
    echo "=== Existing Targets (targets.js) ==="
    # Extract target IDs using grep
    TARGET_IDS=$(grep -oE "id:\s*['\"]([^'\"]+)['\"]" "$PROJECT_PATH/targets.js" | sed "s/id:\s*['\"]//g" | sed "s/['\"]//g")
    if [[ -n "$TARGET_IDS" ]]; then
        echo "$TARGET_IDS" | while read -r id; do
            echo "  - $id"
        done
    else
...
# List existing tasks.js content (for cross-reference)
...
echo ""
echo "=== Summary ==="
echo "Project is ready for target configuration."
